...
st.set_page_config(page_title="GNS 425", page_icon=":guardsman:", layout="wide")
...
materials =st.selectbox("Materials",["SELECT","ONE","TWO","THREE","FOUR"])
if materials == "ONE":
...
if materials == "TWO":
    ...
    displaypdf('lecture.pdf')

if materials == "THREE":
    st.info("YOU HAVE SELECTED THE THIRD MATERIAL !")
    displaypdf('case2.pdf')

if materials == "FOUR":
    st.info("YOU HAVE SELECTED THE FOURTH MATERIAL !")
    displaypdf('case1.pdf')
...
df_encoded = pd.get_dummies(df1,columns=['Coursecode','Coursetitle','Subtopic'], drop_first=True)
df_encoded.drop(['id','isObjective'], axis=1, inplace=True)
df_encoded = df1.apply(le1.fit_transform)
...
with st.container():
    ...
    st.success("""  #  40 QUESTIONS QUIZ GENERATED TO TEST YOUR PREPAREDNESS FOR THE LAW OF CONTRACT EXAMS 
...
for i, question in enumerate(unique_question):
    ...
    # user_answer = st.radio("please choOse your answer",options, key=f"question_{i+1}")
...
grouped_df = df1.groupby(['Question', 'Coursecode', 'Coursetitle', 'Subtopic']).size().reset_index(name='number of questions')
